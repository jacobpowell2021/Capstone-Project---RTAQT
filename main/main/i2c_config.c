...
#define I2C_MASTER_FREQ_HZ         30000      // Clock frequency 100kHz
...
esp_err_t VCELL(float* battery_voltage) {
    ...
    if (battery_voltage == NULL) return ESP_ERR_INVALID_ARG;
    ...
    // --- SAddr + W ---
    i2c_master_write_byte(cmd, (BATMON_ADDR << 1) | I2C_MASTER_WRITE, true);  // A

    // --- MAddr ---
    i2c_master_write_byte(cmd, MAddr, true);  // A

    // --- Sr ---
    i2c_master_start(cmd);

    // --- SAddr + R ---
    i2c_master_write_byte(cmd, (BATMON_ADDR << 1) | I2C_MASTER_READ, true);  // A

    // --- Data0 (MSB) ---
    i2c_master_read_byte(cmd, &data[0], I2C_MASTER_ACK);  // A → tells IC to send next byte

    // --- Data1 (LSB) ---
    i2c_master_read_byte(cmd, &data[1], I2C_MASTER_NACK); // N → end of read
    ...
    printf("Raw VCELL bytes: %02X %02X\n", data[0], data[1]);

    uint16_t raw_vcell = ((uint16_t)data[0] << 8) | data[1];

    *battery_voltage = (float)raw_vcell * 78.125 * 0.000001;
...
esp_err_t set_vreset_register(float vreset_voltage, bool disable_comparator)
{
    ...
    // Convert voltage to 7-bit value (40 mV per LSB)
    vreset_raw = (uint8_t)(vreset_voltage / 0.04f) & 0x7F;

    // Combine bits: VRESET[7:1] + Dis bit
    uint8_t reg_value = (vreset_raw << 1) | (disable_comparator ? 1 : 0);

    uint8_t data_wr[2] = {reg_addr, reg_value};

    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (BATMON_ADDR << 1) | I2C_MASTER_WRITE, true);
    i2c_master_write(cmd, data_wr, sizeof(data_wr), true);
    i2c_master_stop(cmd);

    ret = i2c_master_cmd_begin(I2C_MASTER_NUM, cmd, pdMS_TO_TICKS(500));
    i2c_cmd_link_delete(cmd);

    if (ret != ESP_OK) {
        ESP_LOGE("BATMON", "Failed to write VRESET: %s", esp_err_to_name(ret));
        return ret;
    }

    ESP_LOGI("BATMON", "VRESET set to %.2f V (raw=0x%02X), Dis=%d",
             vreset_voltage, reg_value, disable_comparator);
  ...
  // 7-bit I2C address of new battery monitor
static const char *TAG_RSOC = "BATMON_RSOC";



esp_err_t rsoc(float *soc)
{
    if (soc == NULL) return ESP_ERR_INVALID_ARG;

    esp_err_t ret;
    uint8_t MAddr = 0x04;  // SOC register
    uint8_t data[2] = {0};
    ...
    printf("Raw SOC bytes: %02X %02X\n", data[0], data[1]);

    // Combine MSB + LSB
    uint16_t raw_soc = ((uint16_t)data[0] << 8) | data[1];

    // Conversion: 1 LSB = 1/256 %
...
esp_err_t set_RCOMP() {
    ...
    uint8_t data_wr[2] = {MAddr, data[0]};
    ...
    if (ret != ESP_OK) {
        ESP_LOGE("BATMON", "Failed to write RCOMP: %s", esp_err_to_name(ret));
        return ret;
    }
